...
const CasesList = () => {
  ...
  const filteredCases = useMemo(() => {
    ...
    return cases.filter((caseItem) => {
      const haystack = `${caseItem.caseId || ""} ${caseItem.patientName || ""} ${caseItem.caseType || ""} ${caseItem.status || ""}`.toLowerCase();
        ...
        <div className="overflow-hidden rounded-2xl">
          <TableWrapper tableHeader={TABLE_HEADER}>
            {casesListQuery.isLoading ? (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <ActionButton
                          icon={<Edit size={16} />}
                          btnText="Edit"
                          btnClick={() => navigate(`/cases/${caseItem.caseId || caseItem.id}`)}
              ...
              <tr>
                <td className="px-4 py-6 text-center text-sm text-gray-500" colSpan={TABLE_HEADER.length}>
                  {search ? "No cases found matching your search." : "No cases found. Create your first case to get started."}
                </td>
              </tr>
            )}
          </TableWrapper>
